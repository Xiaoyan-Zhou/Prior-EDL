...
def parse_option():
    ...
    parser.add_argument('--result_root', type=str, default='./results/T5/', help='txt path for writting test results')
    ...
    with tqdm(train_loader, total=len(train_loader)) as pbar:
        for i, (input, label, path) in enumerate(pbar):
            ...
            if opt.use_uncertainty:
                y = one_hot_embedding(label, opt.n_ways)
                y = y.to(device)
            ...
            if opt.loss_option == 'CE' or opt.loss_option == 'EDL':
                ...
                for indx, val in enumerate(pred_s.eq(pred_t)):
                    if val.item() == True:# sample in the shared domain
                        if pred_s[indx] == label[indx]:
                            y_prior[indx] = F.softmax(logit_t[indx] / opt.kd_T, dim=0)
                loss = Prior_edl_mse_loss(logit_s, y_prior.float(), target, epoch, opt.n_ways, 5, device)
...
def main(opt):
    ...
    acc_list_test_30 = []
    acc_list_test_45 = []
    #The random seed list here ensures that different methods are experimented on the same sampling dataset
    ...
    if opt.use_uncertainty:
        ...
        print('the loss function is used for uncertainty estimation and classification')
    else:
        criterion_cls = nn.CrossEntropyLoss()
        print('the loss function is cross Entropy, which is used only for classification')
    ...
    for count in count_list:
        ...
        #Use a random seed to ensure that the data set is different for each sample
        trainset = Dataset('all-train', seed_list[count], opt)
        #Set a random seed here to ensure that each training is out of order,
        # that is, you can keep the internal data in a single training out of order,
        # but the same order is the same between different trainings
        setup_seed(50)
        ...
        # When testing uncertainty, set the batch_size to 1
        ...
        #为计算不确定性相关指标进行全面测试,iid15_15grass_15中15grass表示teacher网络在grass 15上获得，iid15表示测试数据为15°俯仰角，15表示15°数据中得到的最优模型
        #独立同分布数据（俯仰角相同）上得到的最优收敛模型预测
        data_test_complex(iid_test_loader, best_model_all_test, device, opt, title='id15_15grass_15')
        data_test_complex(ood_testset_15_loader, best_model_all_test, device, opt, title='ood15_15grass_15')
        data_test_complex(ood_testset_17_loader, best_model_all_test, device, opt,  title='ood17_15grass_15')

        #俯仰角为17°得到的最优收敛模型上的预测
        data_test_complex(iid_testset_17_loader, best_model_test_17, device, opt, title='id17_15grass_17')
        data_test_complex(ood_testset_15_loader, best_model_test_17, device, opt, title='ood15_15grass_17')
        data_test_complex(ood_testset_17_loader, best_model_test_17, device, opt, title='ood17_15grass_17')

        #俯仰角为30°得到的最优收敛模型上的预测
        data_test_complex(iid_testset_30_loader, best_model_test_30, device, opt, title='id30_15grass_30')
        data_test_complex(ood_testset_15_loader, best_model_test_30, device, opt, title='ood15_15grass_30')
        data_test_complex(ood_testset_17_loader, best_model_test_30, device, opt, title='ood17_15grass_30')

        #俯仰角为45°得到的最优收敛模型上的预测
        data_test_complex(iid_testset_45_loader, best_model_test_45, device, opt, title='id45_15grass_45')
        data_test_complex(ood_testset_15_loader, best_model_test_45, device, opt, title='ood15_15grass_45')
        data_test_complex(ood_testset_17_loader, best_model_test_45, device, opt, title='ood17_15grass_45')
