...
def parse_option():
    ...
    parser.add_argument('--loss_option', type=str, default='Prior-EDL',
    ...
    parser.add_argument('--data_path', type=str, default='./data/MSTAR_REAL/fdf8-EOC/30')
    parser.add_argument('--model_path', type=str, default='./Pretrained_models', help='path for pretrained teacher model')
    parser.add_argument('--model_name', type=str, default='30_grass.pth', help='model name for pretrained teacher model')
    parser.add_argument('--result_root', type=str, default='./results/MSTAR/30_grass/', help='txt path for writting test results')
...
def main(opt):
    ...
    if opt.use_uncertainty:
        ...
        print('don not use uncertainty')

    for count in range(opt.sample_times):
        # dataloader
        # train
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            if epoch % 3 == 0:
                ...
                test_15_acc = data_test(iid_testset_15_loader, model_s, device)
                if best_acc_test_15 < test_15_acc and epoch > 45:
                    best_acc_test_15 = test_15_acc
                    best_model_test_15 = copy.deepcopy(model_s)
        ...
        test_15_acc = data_test(iid_testset_15_loader, best_model_test_15, device)
        ...
        acc_list_test_15.append(test_15_acc.cpu())
        acc_list_test_45.append(test_45_acc.cpu())

        data_test_complex(iid_test_loader, best_model_all_test, device, opt, title='id30_30grass_30')
        data_test_complex(ood_testset_15_loader, best_model_all_test, device, opt, title='ood15_30grass_30')
        data_test_complex(ood_testset_17_loader, best_model_all_test, device, opt,  title='ood17_30grass_30')

        data_test_complex(iid_testset_17_loader, best_model_test_17, device, opt, title='id17_30grass_17')
        data_test_complex(ood_testset_15_loader, best_model_test_17, device, opt, title='ood15_30grass_17')
        data_test_complex(ood_testset_17_loader, best_model_test_17, device, opt, title='ood17_30grass_17')

        data_test_complex(iid_testset_15_loader, best_model_test_15, device, opt, title='id15_30grass_15')
        data_test_complex(ood_testset_15_loader, best_model_test_15, device, opt, title='ood15_30grass_15')
        data_test_complex(ood_testset_17_loader, best_model_test_15, device, opt, title='ood17_30grass_15')

        #俯仰角为45°得到的最优收敛模型上的预测
        data_test_complex(iid_testset_45_loader, best_model_test_45, device, opt, title='id45_30grass_45')
        data_test_complex(ood_testset_15_loader, best_model_test_45, device, opt, title='ood15_30grass_45')
        data_test_complex(ood_testset_17_loader, best_model_test_45, device, opt, title='ood17_30grass_45')
...
if __name__ == '__main__':
    ...
    for i in range(len(k_list)):
        ...
        opt.result_path = opt.result_root + str(opt.n_ways)+'way-'+str(opt.k_shots)+'shot'
